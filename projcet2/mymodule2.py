    ...
    def __init__(self, database, username='root', password='', port=3306, server="localhost"):
        ...
        try:
            self.db = mysql.connector.connect(
                host=self.server, user=self.username, passwd=self.password, database=self.database, port=self.port)
            ...
            if error.errno == 1049:
                ...
                print(
                    "can not connect to mysql server (check server host name/port no) may be mysql server has not been started")
            elif error.errno == -1:
