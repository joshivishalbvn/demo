...
def ReportsModule(start_date=None,end_date=None):
...
while True:
    ...
    if MainModuleChoise == 1:
        ...
        start_date = start_date.strftime("%Y-%m-%d")
        end_date = end_date.strftime("%Y-%m-%d")
        sql = "select id,teacherid,subjectid,batchid,duration,amount,lecturedate from lecture where  lecturedate>=%s and lecturedate<=%s and teacherid=%s"
        values = [start_date, end_date,id]
        table = db.FetchRow(sql, values)
        print('-'*80)
        print(f"{'id'} {'teacherid':10} {'subjectid':10} {'batchid':8} {'duration':8} {' ':3} {'amount':10} {'lecturedate'}")
        print('-'*80)
        ...
        for row in table:
            ...
            print("-"*80)
            GrandTotal = GrandTotal+row['amount']
        print(f"GrandTotal = {GrandTotal:39}")


    elif MainModuleChoise == 7:
            while True:
                ...
                ReportsChoise = int(input())

                if ReportsChoise == 1:
                    start_date=input("Enter Start Date")
                    end_date=input("Enter End Date")
                    ReportsModule(start_date=start_date,end_date=end_date)

                elif ReportsChoise== 2:
                    DisplayBatch(all=all)
                    id=int(input("Enter Batch Id To Get Total"))
                    TotalAmountModule()
                elif ReportsModule == 0:
                    break
                else:
                    print("Invalid Choise")
